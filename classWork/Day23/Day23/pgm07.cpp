#include <iostream>
using namespace std;

typedef struct Employee {
    int empID;
    string empName;
} EMP;

typedef struct Period {
    
    int days;
} PERIOD;

typedef struct Salary {
    double basic;
    double vsal;
    double allowance;
    double gross;
} SAL;

int getEmpDetails(EMP*);
int getPeriodDetails(PERIOD*);
int getSalaryDetails(SAL*);
void calculateGross(SAL*,PERIOD*);
void dispPayroll(EMP, PERIOD, SAL);

int main() {
    EMP e[3];
    PERIOD p[3];
    SAL s[3];

    for (int i = 0; i < 3; i++) {
        
        getEmpDetails(&e[i]);
        getPeriodDetails(&p[i]);
        getSalaryDetails(&s[i]);
        calculateGross(&s[i],&p[i]);
    }

    for (int i = 0; i < 3; i++) {
        dispPayroll(e[i], p[i], s[i]);
    }

    return 0;
}

int getEmpDetails(EMP* e) {
    cout << "Enter Name: ";
    cin >> e->empName;
    cout << "Enter ID: ";
    cin >> e->empID;
    return EXIT_SUCCESS;
}

int getPeriodDetails(PERIOD* p) {
   
    cout << "Enter days: ";
    cin >> p->days;
    return EXIT_SUCCESS;
}

int getSalaryDetails(SAL* s) {
    cout << "Enter Basic Salary: ";
    cin >> s->basic;
    cout << "Enter Variable salary: ";
    cin >> s->vsal;
    cout << "Enter Allowance: ";
    cin >> s->allowance;
    return EXIT_SUCCESS;
}

void calculateGross(SAL* s,PERIOD *p) {
    s->gross = (s->basic + s->vsal + s->allowance)*p->days;
}

void dispPayroll(EMP e, PERIOD p, SAL s) {
    cout << "\n===========================================" << endl;
    cout << "Employee ID: " << e.empID << endl;
    cout << "Employee Name: " << e.empName << endl;
    cout << "Pay Period: "  << p.days << endl;
    cout << "Basic: " << s.basic << ", HRA: " << s.vsal << ", Allowance: " << s.allowance << endl;
    cout << "Gross: " << s.gross << endl;
    cout << "===========================================\n";
}
